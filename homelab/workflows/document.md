...
3. **Create or update the component page** under `components/`. Fill all required frontmatter, including a one-line `description`. Set `last_verified` to today. List everything it depends on in its own `depends_on`; that is the only stored edge. Do not touch other components' frontmatter to record the reverse direction, it is derived (`python3 lint.py reverse-deps`).
