...
In Claude Code each workflow is also a skill wrapper under `.claude/skills/`, so `/document`, `/triage`, etc. dispatch the same files deterministically; lint errors when a wrapper and its workflow drift apart.
