...
from .model import (
    GENERATED_MARKER, YAML_FENCE_RE, build_link_index, discover_pages,
    parse_index_pinning, resolve_link, Report,
)
...
def generate_index_body(pages):
    ...
    for section, types in CONFIG["index_sections"]:
        ...
        for p in members:
            ...
            lines.append(f"- [[{link}]] — {desc} {entry_extra(p.fields or {})}".rstrip())
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
